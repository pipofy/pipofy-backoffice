...
Antes de arrancar: copiar `.env.example` → `.env.development` (y `.env.staging` / `.env.production` si hacen falta). `set-env.mjs` genera `src/environments/environment*.ts` a partir de ese archivo — **son archivos generados, no editarlos a mano**. Si no existe el `.env.*`, cae a leer las `NG_*` de `process.env` (es el caso de Render, ver `render.yaml`). El script aborta si falta una clave requerida o si detecta algo que parece secreto.

## Arquitectura

Clean architecture en tres capas, **impuestas por `eslint-plugin-boundaries`** (`eslint.config.js`). Violarlas es error de lint, no una convención:

| Capa | Path / alias | Puede importar |
|---|---|---|
| `domain` | `src/app/core/domain` → `@domain/*` | sólo `domain`. **Prohibido `@angular/*`** — TS puro |
| `data` | `src/app/core/data` → `@data/*` | `domain`, `data` |
| `shared` | `src/app/shared` → `@shared/*` | sólo `shared` (por eso no puede usar `DomainError`) |
| `features/<x>` | `src/app/features/*` → `@features/*` | `domain`, `data`, `shared` — **nunca otra feature** |

Usar siempre los alias de `tsconfig.json`, no rutas relativas largas.
...
- **Idioma**: comentarios, copy de UI y nombres de feature/rutas en **español** (`features/reservas`, `/configuracion`); `core/` en **inglés**.
